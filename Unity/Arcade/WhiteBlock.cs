...
public class WhiteBlock : MonoBehaviour
{
    ...
    {
        if (inTrigger && Input.GetKeyDown(KeyCode.RightArrow))
        {
            ...
            if (spc.transform.localScale.y > 0.6)
            {
                ...
                Score.scoreValue += 20;
                Destroy(pObj, 0.5f);
                ScoreParticle.instance.RightparticlePlay();
                BlockBreakPtc.instance.BreakBlock();
            }
        ...
        else if (inTrigger && Input.GetKeyDown(KeyCode.LeftArrow))
        {
            LifeSystem.health -= 1;
        }
