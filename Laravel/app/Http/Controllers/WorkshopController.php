...
class WorkshopController extends Controller
{
    ...
    public function cutter(Request $request){
        ...
        shell_exec("ffmpeg -i ".$ts." -c copy -ss ".$request->start_sec." -t ".$request->end_sec." -y /var/www/capstone/RhythmTataki/Laravel/public/song/clip/".$request->clip_name.".ogg");
        Storage::disk('s3')->delete('workshop/temporarySound/'.$audioName);

        $s3 = AwsFacade::createClient('s3');
        $s3->putObject([
            'Bucket' => 'capstone.rhythmtataki.bucket',
            'Key' => 'workshop/drumSoundClip/'.\Auth::user()->email.'/'.$request->clip_name.'.ogg',
            'SourceFile' => 'song/clip/'.$request->clip_name.".ogg",
        ]);

        $size = Storage::disk('s3')->size('workshop/drumSoundClip/'.\Auth::user()->email.'/'.$request->clip_name.'.ogg');
